...
function buildArticles(root, out) {
  ...
  const outputDir = path.join(out, "guide");
  ...
  for (const article of manifest) {
    ...
    const bodyPath = path.join(articlesDir, `${slug}.html`);
  ...
  const builtSlugs = new Set(
    manifest.filter((a) => fs.existsSync(path.join(articlesDir, `${a.slug}.html`))).map((a) => a.slug)
  );
