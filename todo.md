...
- [ ] **2. 内部リンクを設計して回遊と評価集中を作る**
  - [ ] `machines/*/index.html` に「関連記事（guide）」を機種タイプ/天井有無で出し分けて追加
  - [ ] `guide/*.html` の本文中に該当 `machines/{id}/` へのリンクを複数配置（文脈に沿って）
  - [ ] トップ `index.html` に「人気機種/注目機種」枠を追加して機種LPへの深いリンクを増やす（カテゴリ導線も検討）

- [ ] **3. “薄い量産”に見えない差別化（E-E-A-T寄せ）**
  - [ ] 機種LPの手書き解説に「この機種の注意点（リセ運用/入力ミス/モデル外要素）」を1〜3点追加
  - [ ] 主要機種（上位10〜20）だけ一次情報/体験要素を増やして差を付ける（全機種一律は不要）

- [ ] **4. Search Console 起点の改善ループを回す**
  - [ ] 表示回数はあるがCTRが低いページの title/description をKW意図に寄せて改善
  - [ ] 平均掲載順位11〜20位のページはFAQ追加・見出し最適化・関連記事導線強化で底上げ
  - [ ] クエリ別に「派生ページを作る/既存ページを強化する」の判断ルールを作る
