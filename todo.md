...
## フェーズ6（アクセス増：優先度 HIGH）— 完了

> 目的：検索流入（ロングテール）と回遊を増やす。機種LPは **単一URL**（`machines/{id}/`）に集約済み。ページ内の `#lp-setting` / `#lp-ceiling` / `#tool` と **guide / 他機種** への内部リンクで回遊を作る。

- [x] **1. ロングテール用の入口ページを増やす（機種×意図別）**
  - [x] 機種LPを「天井」「設定差」「ツール」のブロックに整理し、同一ページ内で意図別に読めるようにする（旧 `ceiling/` 等の別URLは廃止、`vercel.json` で 301）
  - [x] 各ブロックで狙うKWを定義（例：`機種名 天井 何g` / `機種名 狙い目` / `機種名 設定差 初当たり` / `機種名 設定判別`）
  - [x] canonical 方針を決める（**正規URLは `machines/{id}/` のみ**）

- [x] **2. 内部リンクを設計して回遊と評価集中を作る**
  - [x] `guide/*.html` の本文末に「関連機種」ブロックを自動挿入（複数の `machines/{id}/` へのリンクを配置）
  - [x] トップ `index.html` に「アクセスランキング（上位5）」を追加（`data/access-ranking.json` → `app.js` で fetch・描画、GSC 更新は `scripts/update-access-ranking-from-gsc.js`）

- [x] **3. “薄い量産”に見えない差別化（E-E-A-T寄せ）**
  - [x] 機種LPに「注意点（リセ運用/入力ミス/モデル外要素）」を主要機種だけ1〜3点表示して差別化
  - [x] 主要機種（上位10〜20）だけ、運用差・判断ミスしやすい点など“経験寄り”の補足を追加（全機種一律は不要）

- [x] **4. Search Console 起点の改善ループを回す**
...
> 目的：Google の発見・回遊を強化する。**トップ → 各機種**は既存の一覧で担保済み。機種LPは **1機種1URL** に集約したため、**guide ↔ machines** のリンクとトップのアクセスランキングが主な深い導線になる。

- [x] **1. 機種LPの回遊**
  - 旧「他機種」セクションは方針A（2本柱）に合わせて削除済み。関連機種は **guide 記事末の自動ブロック**とトップ一覧で担保。

- [x] **2. ページ内の読み順**
  - 旧「設定 → 天井 → 初心者」の**別URLナビ**は廃止。同一ページ内の目次・`#lp-setting` / `#lp-ceiling` / `#tool` で代替。
...
> - 検出 - インデックス未登録：**多数ページ**（調査時点・旧4URL/機種構成を含む）← 重大だった要因の一つ  
...
#### 原因D（中期）: 機種LPの類似テンプレート量産

- `machines/` は機種数分の **単一URL**（`index.html`）だが、設定表・天井表中心で機種固有テキストが少ないページは「薄い」と判断されやすい。  
- 旧構成の **67機種 × 4URL** は集約・301 済み。今後は **手書き解説・注意点・ページ内導線** で差別化する。  
- → **「検出 - インデックス未登録」の主因の一つ**（中長期で緩和を狙う）
...
- [x] **E1. 機種LPの差別化（旧 ceiling/setting 相当のブロックを単一ページへ統合）**
  - 方針Aにより **1ページ**に `EDITORIAL_BY_ID`・注意点・設定柱／天井柱を集約。旧派生URLは `vercel.json` で 301。

- [x] **E2. noindex 戦略の検討**（オプション）
  - **結論:** 機種LPは **1機種1URL** に集約済みのため、旧派生URL向けの noindex 集中戦略は**不採用**。薄さ対策は本文・注意点・ガイド連携（E1 と同系）で行う。

#### P3（仕上げ）— Search Console での対応確認

- [x] **F1. 修正後に Search Console で URL 検査 → 再クロールリクエスト**
  - 手順は **README の「デプロイ（Vercel）」**（リダイレクト変更直後の URL 検査・インデックス登録リクエスト・`sitemap.xml` 再送信）および **「Search Console（GSC）改善ループ」** に集約。実行はデプロイ後の運用で行う。
...
| E1: コンテンツ充実・URL集約 | 検出-インデックス未登録（中長期） |