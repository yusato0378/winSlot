...
| `scripts/dev/suggestion-sanity.js` | 設定示唆の校正ガードレール。「示唆が1つも出ていない」ことの尤度比を投入済み全機種で測る（要 `npm run build`） |
| `scripts/dev/dist-sandbox.js` | 上記2つが使う共通処理。`dist/` の実物をブラウザ相当の vm で読み込む |
...
AT終了画面などの設定示唆演出の観測回数を入力として受け取り、既存のベイズ設定判別の尤度に乗算します。データを入れた機種だけで動き、**`suggestions` が無い機種は従来どおり**です。投入済みの機種数は `npm run build` の `Suggestions: N / 97 machines` の行、機種の一覧は `node scripts/dev/suggestion-sanity.js` で確認できます（README に機種名を書くと必ず古くなるため置いていません）。
...
**その演出の分母が `trialSource` と一致するかどうか**で決めます。73機種を入れた結果、判断はこの3分類で足ります。

| 分母と `trialSource` の関係 | 扱い | 例 |
|---|---|---|
| 一致（その契機ごとに必ず1回抽選される） | **`true`** ＋デフォルト行 | AT終了画面、ボーナス終了画面 |
| 部分集合・倍数（同系統だがズレる） | **`false`**、デフォルト行は置かない | ATセット開始ごとの扉絵、REG側のカットイン、CZ失敗時のみの示唆 |
| 完全に別系統（頻度が連動しない） | 不採用 | エンディング中のシール・ボイス・サイン |

- **`true`** は多項分布で扱い、**入力しなかった項目は「出なかった」という否定的証拠**になります。
- **`false`** は項目ごとの独立二項。獲得枚数・トロフィー・ボイスのように出ること自体が稀な単発事象もこちら（**獲得枚数系は35グループ中34グループが `false`**）。非排他グループではデフォルト行は尤度に効かないので置きません。
- **同じ抽選を条件で分割したグループは必ず `false`。** `birdie_wing` のイヴ周期／葵周期のカードや `discup_ultraremix` のREG中モード別演出は、1回の試行がどちらか一方でしか発生しません。両方を `true` にすると、各グループが同じ試行回数ぶんのデフォルトを数えて否定的証拠が二重に効きます。
- **同じ画面上の別抽選を2つとも `true` にしない。** `sao2` はAT終了画面とコパンダトロフィーを両方 `true` にしたところ、1枚の画面を見た事実から「何も出なかった」証拠を2回取る形になり、尤度比が 1:280 まで伸びました（トロフィー側を `false` にして 1:29.5）。
...
`favor5`（設定5優遇）だけは `weights` で設定ごとの重みを持ちます。当初は設定1・2の重みが無く、出現率0＝**除外**として扱われていました。「設定5優遇」という文言は設定1・2の否定までは意味しないため、低い重み（0.1 / 0.2）を与えて除外をやめています。
...
node scripts/dev/suggestion-sanity.js  # 校正ガードレール（要 npm run build）
```

`suggestions` を足したら必ず4つとも通してください。特に `baseline.js` は、示唆機能が既存の判別を壊していないことの唯一の担保です。

`suggestion-sanity.js` は、投入済み全機種について「初当たり20回ぶん、どのグループにも示唆が1つも出ていない」の**尤度比（最高設定 : 最低設定）**を測ります。**`exclusive: true` のグループは「入力しなかった項目＝出なかった」を否定的証拠として使うため、これがグループのぶんだけ積み上がります。** 効くのはグループ数だけではなく、**グループ内の非デフォルト項目の率の合計**です（項目が多いほどデフォルトの取り分が小さくなり、デフォルトを引き続けることが強い低設定証拠になる）。

事後確率ではなく尤度比で見るのは、事後確率が機種のスペックと測定点に依存するからです（初当たりの重い機種は示唆と無関係に設定6が低く出るので、機種間で比べられません）。**尤度比が 1:1000 を超える**、または**1グループの示唆出現率が30%を超える**と落ちます。落ちたらその機種の項目を減らすか、複数機種で出ているなら `data/suggestion-ranks.json` の `base` / `from` / `to` を一律で下げてください。1:100 を超えた機種は落ちませんが「示唆を1つも見ていないだけで設定が決まる」水準なので、出力の末尾に見直し候補として並びます。
