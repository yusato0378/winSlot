...
`suggestion-sanity.js` は、投入済み全機種について「6000G・初当たり20回・デフォルト行を20回入力」した事後確率を測ります。**`exclusive: true` のグループは「入力しなかった項目＝出なかった」を否定的証拠として使うため、これがグループのぶんだけ積み上がります。** 効くのはグループ数だけではなく、**グループ内の非デフォルト項目の率の合計**です（項目が多いほどデフォルトの取り分が小さくなり、デフォルトを引き続けることが強い低設定証拠になる）。実測でも、3グループ17項目の `sengoku_otome5`（設定6が0.7%まで落ちる）は、グループ数がより多く項目の少ない構成より強く効きます。 ランクの `base` は「1グループの示唆が設定6で1割前後」で校正した数値なので、機種を足していけば必ずどこかで崩れます。設定6が0.5%未満または設定1が60%超で落ちるので、そうなったら該当機種のグループを減らすか、複数機種で出ているなら `data/suggestion-ranks.json` の `base` / `from` / `to` を一律で下げてください。
